...
# --- CORRECTED THIS LINE: Changed to import lowercase names ---
try:
    from chatbot_backend import agent_executor, retriever # Changed AGENT_EXECUTOR, RETRIEVER to agent_executor, retriever
...
async def get_relevant_documents(query: str) -> List[Document]: # Made async
    ...
    """
    if retriever is None: # Corrected to lowercase retriever
        raise ValueError("Retriever is None. Ensure chatbot_backend.py initializes it correctly.")
    return await retriever.ainvoke(query) # Corrected to lowercase retriever and ainvoke
...
async def predict_agent_run_for_eval(example: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        response = await agent_executor.ainvoke({"input": question}) # Corrected to lowercase agent_executor and ainvoke
...
dataset_name = "My_Financial_Literacy_QA_Dataset" # Ensure this matches your Langsmith dataset name
